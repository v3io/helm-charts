...
keycloak_initialize() {
    ...
    if [[ "$KC_DB" = "mysql" || -z "${KC_DB_URL:-}" ]]; then
        info "Using Environment variables for database configuration: host=${KEYCLOAK_DATABASE_HOST}, port=${KEYCLOAK_DATABASE_PORT}"
        db_host="$KEYCLOAK_DATABASE_HOST"
        db_port="$KEYCLOAK_DATABASE_PORT"
    elif [[ "$KC_DB" = "postgres" ]]; then
        # Extract host and port from KC_DB_URL
        info "Using KC_DB_URL for database configuration: url=${KC_DB_URL}"
    ...
    if [[ -n "$db_host" && -n "$db_port" ]]; then
        # Wait for database
        info "Trying to connect to database server $db_host..."
        if ! retry_while "wait-for-port --host $db_host --timeout 10 $db_port" "$KEYCLOAK_INIT_MAX_RETRIES"; then
            ...
            info "Found database server listening at $db_host:$db_port"
        fi
    fi
